...
1. **Set up a structured Claude session.** *(claude.ai)* No project directory exists yet — ideation happens in claude.ai, not in a repo. claude.ai cannot read your local filesystem; you have to bring the templates to it. Three paths:
   ...
   claude.ai does not currently have a first-party Bitbucket connector. If your scaffold lives on Bitbucket and you want live access, the practical options are: mirror to a private GitHub repo and use the GitHub connector, set up a self-hosted MCP server exposing Bitbucket's API (technical), or rely on file upload (static).
