...
- **Every gate records the md5 of the files it judged.** A browser pass records the md5 of every file the browser fetched, at the START and again at the END of the run (the second catches a tree that shifted under the gate). Put them in the gate's own evidence file, not in prose — and that evidence file lives under an **ignored** path such as `artifacts/logs/`, never a tracked one: the pre-commit gate is read-only over the tree (v0.14.10, "Pre-commit verification gate" below), and a tracked evidence file the gate rewrites is exactly the footprint it refuses.
...
**The gate is read-only over the tree (v0.14.10).** The rule: a verify gate is read-only over tracked files and writes nothing untracked; gate output belongs under an ignored path. The wrapper proves it rather than trusting it — `git status` before and after the gate command, and whatever appeared is the gate's *footprint*. A footprint is restored (tracked paths back to HEAD, untracked ones deleted) and the gate is **red even when its command exited 0**: `artifacts/phase-verify-failed.json` gains `gate_footprint` (the paths), `gate_footprint_rule` and `gate_footprint_recipe`, and the next pass fixes the gate before anything else (`CONTINUE_PROMPT.txt` step 2). The recipe: write the gate's output under `artifacts/logs/` (ignored in every scaffolded project) or another ignored path; a measurement the project wants committed is produced in the phase's own work, not by the gate; a formatter runs in check mode here, never with a write flag. Dirt present *before* the gate is the session's staged work and its staged bytes are never touched. A gate that rewrites a file the session itself edited — a formatter with a write flag, or a tracked evidence file the session re-measured in its turn that the gate re-measures again — is caught the same way: the commit sites stage everything before the gate, so an unstaged change that appears on a session path across the gate is the gate's, restored to the staged bytes and red. Why a rule and not an allowance: every xmeo landing from iteration 50 to 58 rested dirty because its gate re-measured three tracked evidence files after the completion commit had staged them, and an operator committed the noise by hand at every landing; a fleet audit found that one writer (since moved to an ignored path) and nothing else. A kill inside the gate (during the command, or after it returned and before the restore) is settled at the next loop start, before any turn: the before-snapshot is kept in the transient boundary record until the restore has run, so whatever appeared is restored and recorded red then (no breaker attempt is spent for the settlement; the live re-run that follows spends one) — the gate's dirt is never staged as the session's work, and a fixed gate lands clean. Nothing but the loop may touch a tree the loop left mid-gate: a hand edit made between the kill and the next start is restored with the rest.
...
**`final_phase` (v0.14.5, optional):** set `"final_phase": true` on the approval that closes the project's last phase. The loop then treats the boundary as final: if the session has not also written `artifacts/project-complete.json`, the loop **records the completion itself** from the approval (summary and `deferrals` carried, `recorded_by` naming the step) and lands it in the same sequence — so "approved but the completion was never recorded" cannot survive a kill (2026-09-08 06:22). Writing your own completion record alongside the approval is still the richer choice (walk the change-request's asks, name limitations); the flag guarantees the record exists either way. Without the flag, behavior is unchanged: the next turn writes the completion. A landed `final_phase` approval is honoured only while no commit since it landed has touched `project-complete.json`, so a project that completed and was then resumed is never re-recorded complete; the one shape this cannot tell apart is an operator re-landing the approval with *different* content after the resumption (that reads as a fresh final approval) — do not hand-edit a resumed project's approval to say `final_phase: true`.

**Deferral keys are committed, never hashed (v0.14.5):** before an approval-class artifact is committed, the loop gives every `deferrals` entry a stable `key` — the explicit one if present, else the cited `AC#n`, else a slug of the item's first words (`polish-the-lobby-animation-timing`). An entry with neither `key` nor `item` text, or a `deferrals` value that is not an array of objects, is left as written with a WARN in the session log and dropped by the supervisor's reader (a refusal here would be blind — nothing on disk would tell the next turn why its commit kept failing). A `deferrals` of `null` counts as absent. A supervisor therefore never mints a hash for a deferral it can name.
